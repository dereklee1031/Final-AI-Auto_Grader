...
Inputs:
  --retrieval-jsonl     Path to retrieval_results.jsonl  (from `retrieve` mode)
  --chunks-jsonl        Path to chunks.jsonl             (for ordered student content)
  --student-path        Optional substring to filter to one student file
  --out-dir             Output directory (default: alongside retrieval-jsonl)
  --model               OpenAI model (default: gpt-4o)
  --max-lecture-chars   Max lecture context chars sent to LLM (default: 6000)
  --max-student-chars   Max student content chars sent to LLM  (default: 8000)
    ...
    for row in rows:
        ...
        for doc, meta in zip(docs, metas):
            ...
            if doc:
                ...
                doc_meta.setdefault(doc, meta or {})
...
Your job:
  ...
  - For each question/section, evaluate the student's answer against the lecture concepts.
...
def main() -> int:
    ...
    if not retrieval_jsonl.exists():
        raise SystemExit(f"retrieval_results.jsonl not found: {retrieval_jsonl}")
    ...
    student_text, student_chunks = load_student_content(chunks_jsonl, args.student_path)
    if not student_text:
        raise SystemExit("No student content found. Check --student-path filter or chunks.jsonl.")
    ...
    lecture_context = load_lecture_context(retrieval_jsonl, args.student_path, args.max_lecture_chars)
    ...
    if not result.get("parse_error"):
        ...
        print("\nWARNING: Could not parse JSON from Claude response. Raw output saved in grades.json.")
